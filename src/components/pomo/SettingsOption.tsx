...
export default function SettingsOption(props: Props) {
  ...
  switch (props.type) {
    case "hms":
      input = <input className="teal-shadow" type="tel" pattern="[0-9]{2}h [0-9]{2}m [0-9]{2}s" name={props.id} placeholder={props.ph}/>;
      break;
    case "integer":
      input = <input className="purple-shadow" type="number" min="0" max="999" name={props.id} placeholder={props.ph}/>;
      break;
    case "color":
    ...
    case "font":
      ...
      input = <select className="velvet-shadow" name={props.id} placeholder={props.ph}>{array}</select>
      break;
  }
  return (
